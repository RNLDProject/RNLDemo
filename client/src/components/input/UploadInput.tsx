...
const UploadInput: FC<UploadInputProps> = ({
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      if (acceptedFiles && acceptedFiles.length > 0) {
        ...
        // Inalis natin ang setPreview dito para sa useEffect na lang lahat
  ...
  useEffect(() => {
    ...
    // SAFETY CHECK: Dito maiiwasan ang white screen
    ...
    // CLEANUP: Binubura ang temporary URL sa memory para hindi mag-crash
    return () => {
      if (objectUrl) {
  ...
  return (
    <>
      ...
      <div
        className={`transition border border-gray-300 border-dashed cursor-pointer rounded-lg hover:border-blue-900 ${
          errors && errors.length > 0 ? "mb-0" : "mb-4"
        }`}
      >
        <div
          {...getRootProps()}
          className={`rounded-lg border-dashed border-gray-300 p-7 lg:p-10 ${
            isDragActive ? "border-blue-600 bg-gray-100" : "border-gray-300 bg-gray-50"
          }`}
        >
            ...
            {preview ? (
              <div className="mb-[22px] flex justify-center">
                <img
                  src={preview}
                  alt="Preview"
                  className="w-[68px] h-[68px] rounded-full object-cover border-2 border-blue-600"
                />
              </div>
            ) : (
              <div className="mb-[22px] flex justify-center">
